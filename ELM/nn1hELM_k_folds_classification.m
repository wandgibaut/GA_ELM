...
for fold=1:k,
    ...
    for i=1:n_times,
        ...
        %for j=1:length(c),
            ...
            S_estimado = [tanh([Hv]*OutputWeight)];
...
%             ValidatingAccuracy=sqrt(mse(Sv - S_estimado));               %   Calculate training accuracy (RMSE) for regression case
            ...
            S_estimado = defineClass(S_estimado);
            CER = classError(Sv,S_estimado);
            ValidatingAccuracy = sum(CER)/length(CER);
            if ValidatingAccuracy < FinalValidatingAccuracy,

                %%%%%%%%%%% Calculate the training accuracy
                S_estimado = [tanh([H]*OutputWeight)];                             %   S_estimado: the actual output of the training data
%                 TrainingAccuracy=sqrt(mse(S - S_estimado));               %   Calculate training accuracy (RMSE) for regression case
                ...
                S_estimado = defineClass(S_estimado);
                CERt = classError(S,S_estimado);
                FinalTrainingAccuracy = sum(CERt)/length(CERt);
